$(document).ready(function() {
    $('#category-table').DataTable( {
        ...
        "ajax": {
            "url": baseUrl+"/category/get-list-pageable",
            "data": {
                "categoryId": cateId
            },
            "dataSrc": function(json) {
                ...
                ret.forEach(function (ele) {
                    ...
                    if (json.operator.view == 1) {
                        ele.operator += '<a class="btn-op" title="查看" data-url="'+baseUrl+'/category/get-detail/' + ele.id + '"><i class="fa fa-eye"></i></a>';
                    }
                    if (json.operator.edit == 1) {
                        ele.operator += '<a class="btn-op" title="编辑" data-url="'+baseUrl+'/category/edit/' + ele.id + '"><i class="fa fa-edit"></i></a>';
                    }
                    if (json.operator.delete == 1) {
                        ele.operator += '<a class="btn-op" title="删除" data-url="'+baseUrl+'/category/delete/' + ele.id + '"><i class="fa fa-cut"></i></a>';
                    }
                });
                return ret;
             }
        ...
        "columnDefs": [
            {
                "render": function ( data, type, row ) {
                    return '<a href="'+baseUrl+'/category/list/' + row["id"] + '">'+ data +'</a>';
                },
                "targets": 2
            }
        ]
