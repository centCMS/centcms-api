$(document).ready(function () {
    $('#submit-div').css("height", $('#schema-div').css('height'));
    var formObject = {
        schema: {
            example: {
                title: '从示例开始使用JSON Form',
                type: 'string',
                'enum': [
                    'gettingstarted',
                    'schema-basic',
    ...
    var loadExample = function (example) {
        $.ajax({
            url: staticUrl + '/jsonform-2.1.6/playground/examples/' + example + '.json',
            dataType: 'text'
        }).done(function (code) {
    ...
    $('#schema-form').submit(function(){
        ...
        var list = $("#schema-form").serializeObject();
        list[token[0]] = token[1];
        var values = $('#form').jsonFormValue();
        eval('var greatform = ' + values.greatform);
        var schema = JSON.stringify(greatform, function(key, val) {
        ...
        list['content'] = schema;
        $.ajax({
            url: baseUrl + '/schema-template/create',
            dataType: "json",
            type: 'POST',
            data: list
        }).done(function (data) {
